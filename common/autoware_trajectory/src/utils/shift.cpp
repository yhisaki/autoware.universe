  ...
  const double arc_length, const double shift_length, const bool offset_back)
{
  ...
  // If offset_back is false, reverse the lateral shift
  if (!offset_back) {
    std::reverse(base_lateral.begin(), base_lateral.end());
  }
  ...
  const ShiftParameters & shift_parameters)
{
  ...
  if (initial_vel < 1.0e-5 && used_lon_acc < acc_threshold) {
    ...
    return get_base_lengths_without_accel_limit(arc_length, shift_length, offset_back);
  }
  ...
  if (max_lateral_acc < shift_parameters.lateral_acc_limit) {
    ...
    return get_base_lengths_without_accel_limit(
      target_arclength, shift_length, initial_vel, used_lon_acc, total_time, offset_back);
  }
  ...
  if (jerk_time < 0.0 || accel_time < 0.0 || lateral_jerk < 0.0 || (jerk_time / total_time) < 0.1) {
    ...
    return get_base_lengths_without_accel_limit(target_arclength, shift_length, offset_back);
  }
  ...
  // If offset_back is false, reverse the lateral shifts
  if (!offset_back) {
    std::reverse(base_lat.begin(), base_lat.end());
  }
  ...
  const ShiftInterval & shift_interval, const ShiftParameters & shift_parameters)
{
  ...
  interpolator::CubicSpline cubic_spline;
  bool success = cubic_spline.build(base_lon, base_lat);
  ...
  for (size_t i = 0; i < bases.size(); ++i) {
    ...
    if (shift_interval.start <= s && s <= shift_interval.end) {
      shift_lengths->at(i) += cubic_spline.compute(s - shift_interval.start);
    }
    if (shift_interval.end < s) {
